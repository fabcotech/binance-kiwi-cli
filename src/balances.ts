...
export const printBalances = async (symbols: string[], usdSymbol: string) => {
  ...
  if (symbols.length) {
    for (const symbol of symbols) {
      const priceUsd = parseFloat((await getPriceTicker(`${symbol}${usdSymbol}`)).price);
      let b = 0;
      try {
        b = parseFloat(balances.balances.find((b: any) => [symbol, symbol.toUpperCase()].includes(b.asset)).free);
      } catch (err) {
        // do nothing;
      }
      let usdAmount = '';
      ...
      s += `\n${symbol.padEnd(5, ' ')} ${b.toString().padEnd(10, ' ')} ${usdAmount}`;
      obj[symbol] = { quantity: b, usdAmount: usdAmountN };
    }
  } else {
    for (const bal of balances.balances || []) {
      const priceUsd = parseFloat((await getPriceTicker(`${bal.asset}${usdSymbol}`)).price);
      let b = 0;
      try {
      ...
      s += `\n${bal.asset.toUpperCase().padEnd(5, ' ')} ${b.toString().padEnd(10, ' ')} ${usdAmount}`;
