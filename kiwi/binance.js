    ...
    return __generator(this, function (_c) {
        switch (_c.label) {
            case 0:
                ...
                console.log(__assign({ symbol: symbol, side: side, timestamp: new Date().getTime(), type: 'MARKET' }, (side === 'sell' ? { quantity: balance } : { quoteOrderQty: balance })));
                qs = querystring_1.default.stringify(__assign({ symbol: symbol, side: side, timestamp: new Date().getTime(), type: 'MARKET' }, (side === 'sell' ? { quantity: balance } : { quoteOrderQty: balance })));
                signature = (0, exports.signatureBinanceApi)(qs);
